...
EOF
			...
			echoContent green "    https://api.mohuajz.top/qr/?text=&size=300=vless%3A%2F%2F${id}%40${currentHost}%3A${currentDefaultPort}%3Fencryption%3Dnone%26security%3Dxtls%26type%3Dtcp%26${currentHost}%3D${currentHost}%26headerType%3Dnone%26sni%3D${currentHost}%26flow%3Dxtls-rprx-direct%23${email}\n"
...
EOF
			...
			echoContent green "    https://api.mohuajz.top/qr/?text=&size=300=vless%3A%2F%2F${id}%40${currentHost}%3A${currentDefaultPort}%3Fencryption%3Dnone%26security%3Dxtls%26type%3Dtcp%26${currentHost}%3D${currentHost}%26headerType%3Dnone%26sni%3D${currentHost}%26flow%3Dxtls-rprx-splice%23${email/direct/splice}\n"
...
EOF
			...
			echoContent green "    https://api.mohuajz.top/qr/?text=&size=300=vless%3a%2f%2f${id}%40${currentHost}%3a${currentDefaultPort}%3fsecurity%3dtls%26encryption%3dnone%26host%3d${currentHost}%26headerType%3dnone%26type%3dtcp%23${email}\n"
		fi
...
EOF
		echoContent green "    https://api.mohuajz.top/qr/?text=&size=300=vmess://${qrCodeBase64Default}\n"
...
EOF
		...
		echoContent green "    https://api.mohuajz.top/qr/?text=&size=300=vless%3A%2F%2F${id}%40${currentAdd}%3A${currentDefaultPort}%3Fencryption%3Dnone%26security%3Dtls%26type%3Dws%26host%3D${currentHost}%26sni%3D${currentHost}%26path%3D%252f${currentPath}ws%23${email}"
...
EOF
		...
		echoContent green "    https://api.mohuajz.top/qr/?text=&size=300=vless%3A%2F%2F${id}%40${currentAdd}%3A${currentDefaultPort}%3Fencryption%3Dnone%26security%3Dtls%26type%3Dgrpc%26host%3D${currentHost}%26serviceName%3D${currentPath}grpc%26path%3D${currentPath}grpc%26sni%3D${currentHost}%26alpn%3Dh2%23${email}"
	...
	if [[ -n "${configPath}" ]]; then
		...
		if [[ -n $(ls /etc/v2ray-agent/subscribe/) ]]; then
			find /etc/v2ray-agent/subscribe/* | while read -r email; do
				...
				echoContent yellow "在线二维码:https://api.mohuajz.top/qr/?size=300x300&data=https://${currentDomain}/s/${email}\n"
