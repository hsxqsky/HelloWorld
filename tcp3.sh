...
#	Version: 10.0.8.0
#	Author: 失落的梦
#	bash <(curl -s -L https://github.com//wurendi001/shell/tcp2.sh)
#=================================================
...
sh_ver="10.0.8.0"
github="raw.githubusercontent.com/wurendi001/shell/main"
raw.githubusercontent.com/wurendi001/shell/main/tcp2.sh
imgurl=""
headurl=""
...
Update_Shell() {
 local shell_file="$(readlink -f "$0")"
  bash <(curl -s -L https://raw.githubusercontent.com/wurendi001/shell/main/tcp3.sh)
 ...
 bash <(curl -s -L https://raw.githubusercontent.com/wurendi001/shell/main/tcp3.sh)
  ...
  local md5_remote="$(md5sum /tmp/tcp3.sh | awk '{print $1}')"

  if [ "$md5_local" != "$md5_remote" ]; then
    # 替换本地脚本文件
    cp "/tmp/tcp3.sh" "$shell_file"
    chmod +x "$shell_file"
...
start_menu() {
  clear
  echo && echo -e " 失落的梦 TCP加速 一键安装管理脚本 ${Red_font_prefix}[v${sh_ver}] 不卸内核${Font_color_suffix} 母鸡慎用
 ...
 ${Green_font_prefix}11.${Font_color_suffix} 使用BBR1+FQ加速		${Green_font_prefix}12.${Font_color_suffix} 使用BBR+FQ_PIE加速 
