...
warpv4=$(curl -s4m8 ipget.net | grep warp | cut -d= -f2)
warpv6=$(curl -s6m8 ipget.net | grep warp | cut -d= -f2)
if [[ $warpv4 =~ on|plus ]] || [[ $warpv6 =~ on|plus ]]; then
	wg-quick down wgcf >/dev/null 2>&1
	systemctl stop warp-go >/dev/null 2>&1
	ipv4=$(curl -s4m8 ipget.net)
	ipv6=$(curl -s6m8 ipget.net)
...
outputVmess() {
  ...
  \"tls\":\"\"
	...
	echo -e "   ${BLUE}vmess二维码链接:${PLAIN} $GREEN$qrlink$PLAIN"
}
...
outputTrojan() {
	if [[ "$xtls" == "true" ]]; then
		...
		qrlink="https://api.qrserver.com/v1/create-qr-code/?data=${link}size=300x300${link}"
		...
		echo -e "   ${BLUE}Trojan二微码链接:${PLAIN}$RED$link$PLAIN"
	else
		link="trojan://${password}@${domain}:${port}#"
		qrlink="https://api.qrserver.com/v1/create-qr-code/?data=${link}size=300x300${link}"
		echo -e "   ${BLUE}IP/域名(address): ${PLAIN} ${RED}${domain}${PLAIN}"
...
showInfo() {
	...
	if [[ "$vless" == "false" ]]; then
		...
		if [[ "$xtls" == "true" ]]; then
			...
			link="vless://${uid}@${domain}:${port}?encryption=none&security=tls&type=tcp&host=${domain}&headerType=none#VLESS_TCP_TLS"
			qrlink="https://api.qrserver.com/v1/create-qr-code/?size=400x400&data=vless%3A%2F%2F${uid}%40${domain}%3A${port}%3Fencryption%3Dnone%26security%3Dtls%26type%3Dtcp%26host%3D${domain}%26sni%3D${domain}%26path%3D${wspath}%23VLESS_TCP_TLS"
			
			echo -e " ${BLUE}vless链接:${PLAIN} $YELLOW$link$PLAIN"
			echo -e " ${BLUE}vless二维码链接:${PLAIN} $GREEN$qrlink$PLAIN"
		else
			echo -e " ${BLUE}IP(address): ${PLAIN} ${RED}${IP}${PLAIN}"
			echo -e " ${BLUE}端口(port)：${PLAIN}${RED}${port}${PLAIN}"
			echo -e " ${BLUE}id(uuid)：${PLAIN}${RED}${uid}${PLAIN}"
...
######################################### 通用格式(VLESS+WS+TLS) 
			...
			link="vless://${uid}@${domain}:${port}?encryption=none&security=tls&type=ws&host=${domain}&sni=${domain}&path=${wspath}#VLESS_WS_TLS"
			qrlink="https://api.qrserver.com/v1/create-qr-code/?size=400x400&data=vless%3A%2F%2F${uid}%40${domain}%3A${port}%3Fencryption%3Dnone%26security%3Dtls%26type%3Dws%26host%3D${domain}%26sni%3D${domain}%26path%3D${wspath}%23VLESS_WS_TLS"
			echo -e " ${BLUE}vless链接:${PLAIN} $YELLOW$link$PLAIN"
			echo -e " ${BLUE}vless二维码链接:${PLAIN} $GREEN$qrlink$PLAIN"
		fi
	fi
