...
ss_link_qr(){
	...
	SSurl="ss://${SSbase64}"
	SSQRcode=	qrlink="https://api.qrserver.com/v1/create-qr-code/?data=${SSurl}"&size=300${link}
...
Set_config_forbid(){
	...
	echo -e "${Tip} 禁止的端口：例如不允许访问 23端口，用户就无法通过SSR代理访问 端口23了，如果禁止了 80,443 那么用户将无法正常访问 http/https 网站。
封禁单个端口格式: 25
封禁多个端口格式: 23,465
封禁  端口段格式: 233-266
