...
function ScriptTitle() {
    ...
    if [[ "$language" == "e" ]]; then
        ...
        echo ""
        echo -e "${Font_Purple}[流媒体平台及游戏区域限制测试]${Font_Suffix}"
        echo -e "${Font_Purple}脚本适配OS: CentOS 6+, Ubuntu 14.04+, Debian 8+${Font_Suffix}"
        echo ""
        echo -e " ** 测试时间: $datetime "
        echo ""
    fi
}
ScriptTitle
...
function Start() {
    if [[ "$language" == "e" ]]; then
        ...
        echo -e "${Font_Purple}请选择检测项目，直接按回车将进行全区域检测${Font_Suffix}"
		...
		echo -e "${Font_SkyBlue}输入数字  [ 7]: [ 跨国平台+韩国平台 ]检测${Font_Suffix}"
        echo -e "${Font_SkyBlue}输入数字  [ 8]: [跨国平台+大洋洲平台]检测${Font_Suffix}"
        echo -e "${Font_SkyBlue}输入数字  [ 9]: [跨国平台+东南亚平台]检测${Font_Suffix}"
        echo -e "${Font_SkyBlue}输入数字  [10]: [   只进行跨国平台  ]检测${Font_Suffix}"
        echo -e "${Font_SkyBlue}输入数字  [11]: [   体育直播平台    ]检测${Font_Suffix}"
		echo -e "${Font_Purple}输入数字  [ 0]: [     退出脚本      ]检测${Font_Suffix}"
